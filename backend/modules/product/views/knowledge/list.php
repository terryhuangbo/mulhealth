...
<head>
    ...
    <script>
        _BASE_LIST_URL =  "<?php echo yiiUrl('product/knowledge/list') ?>";
    </script>
</head>
    ...
    $(function () {
        ...
        BUI.use(['bui/grid', 'bui/data'], function (Grid, Data) {
            ...
            var store = new Data.Store({
                ...
                root: 'knowledgeList',//数据返回字段,支持深成次属性root : 'data.records',
                totalProperty: 'totalCount',//总计字段
                pageSize: 10// 配置分页数目,
            });
            var grid = new Grid.Grid({
                render: '#knowledge_grid',
                idField: 'id', //自定义选项 id 字段
                selectedEvent: 'click',
                columns: [
                    {title: '序号', dataIndex: 'id', width: 80, elCls : 'center'},
                    {title: '知识名称', dataIndex: 'title', width: 90, elCls : 'center'},
                    {
                        title: '图片',
                        width: 140,
                        elCls : 'center',
                    ...
                    {title: '知识标签', dataIndex: 'tags', width: 80, elCls : 'center'},
                    {title: '知识状态', dataIndex: 'status_name', width: 80, elCls : 'center'},
                        ...
                        renderer: function (v, obj) {
                            if(obj.status == 1){
                                return "<a class='button button-info' onclick='showInfo(" + obj.id + ")'>查看</a>" +
                                    "<a class='button button-success page-action' title='编辑产品知识' href='/product/knowledge/update/?id="+ obj.id +"' data-href='/product/knowledge/update/?id="+ obj.id +"' >编辑</a>" +
                                    " <a class='button button-danger' onclick='offShelf(" + obj.id + ")'>禁用</a>";
                            }else if(obj.status == 2){
                                return "<a class='button button-info' onclick='showInfo(" + obj.id + ")'>查看</a>" +
                                    "<a class='button button-success page-action' title='编辑产品知识信息' data-href='/product/knowledge/update/?id="+ obj.id +"' >编辑</a>" +
            ...
            grid.render();
            $("#knowledge_grid").data("BGrid", grid);
...
<script>
    ...
    function searchCases() {
        ...
        var store = $("#knowledge_grid").data("BGrid").get('store');
    ...
    function ajax_change_status(id, status, callback){
        var param = param || {};
        param.id = id;
        param.knowledge_status = status;
        $._ajax('<?php echo yiiUrl('product/knowledge/ajax-change-status') ?>', param, 'POST','JSON', function(json){
