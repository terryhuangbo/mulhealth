...
<form class="form-horizontal bui-form bui-form-field-container" id="wechat_info_form" aria-disabled="false"
    ...
    <input name="uid" v-role="proj-id" type="hidden" class="bui-form-field" aria-disabled="false"
           value="<?php echo $wechat['uid'] ?>" style="display: none;">

    <div class="control-group">
        <label class="control-label">手机号码：</label>
        <div class="controls">
            <input name="mobile" class="input-normal control-text bui-form-field" data-rules="{required:true}"
                   type="text" v-role="proj-name" aria-disabled="false" value="<?php echo $wechat['mobile'] ?>">
        </div>
    </div>
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <input name="email" class="input-normal control-text bui-form-field" data-rules="{required:true}"
                   type="text" v-role="proj-name" aria-disabled="false" value="<?php echo $wechat['email'] ?>">
        </div>
    </div>
...
<script>
    function saveInfo(){
        var param = $._get_form_json("#wechat_info_form");
        $._ajax('/wechat/wechat/ajax-save', param, 'POST', 'JSON', function(json){
            if(json.code > 0){
                BUI.Message.Alert(json.msg, function(){
                    window.location.href = '<?php echo yiiUrl('wechat/wechat/list') ?>';
                }, 'success');
